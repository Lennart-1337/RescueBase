...
export function DataTable<Row>(props: DataTableProps<Row>) {
  ...
  return <div className="data-table">
    ...
    <table><colgroup>{selectable ? <col className="data-table-selection-column" /> : null}{columns.map((column) => <col key={column.id} style={{ width: column.width }} />)}</colgroup><thead><tr>
      {selectable ? <th scope="col"><input aria-label="Alle sichtbaren Zeilen auswählen" checked={selectableIds.length > 0 && selectedOnPage.length === selectableIds.length} onChange={toggleAll} type="checkbox" /></th> : null}
    ...
    </tr></thead><tbody>{props.rows.map((row) => { const id = props.getRowId(row); return <tr aria-selected={props.selectedRowId === id} data-clickable={Boolean(props.onRowClick)} key={id} onClick={() => props.onRowClick?.(row)}>{selectable ? <td onClick={(event) => event.stopPropagation()}><input aria-label={`${id} auswählen`} checked={props.selectedIds.includes(id)} onChange={() => toggleSelection(id)} type="checkbox" /></td> : null}{columns.map((column) => <td key={column.id}>{column.render(row)}</td>)}</tr>; })}</tbody></table>
  </div>;
}
