        ...
        {props.users.map((entry) => (
          <div className="table-row user-row" key={entry.id}>
            ...
            <RowActions>
              ...
              <Button className="user-toggle-button" disabled={props.isSubmitting} onClick={() => props.onToggle(entry.id, !entry.active)} type="button" variant={entry.active ? "danger" : "secondary"}>{entry.active ? "Deaktivieren" : "Aktivieren"}</Button>
