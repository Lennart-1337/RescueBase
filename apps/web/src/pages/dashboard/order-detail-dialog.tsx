...
export function OrderDetailDialog(props: { batches: Batch[]; error: Error | null; formatReason: (reason: string) => string; formatStatus: (status: string) => string; isOpen: boolean; isSubmitting: boolean; onCancel: () => void; onClose: () => void; onFulfill: (items: Array<{ itemId: string; batchId: string; quantity: number }>) => void; onStart: () => void; order: ReplenishmentOrder | null; pdfHref?: string; selectedBatchQuantity: (batches: Batch[], batchId: string) => number; }) {
  ...
  const showStart = props.order?.status === "OPEN";
  const showCancel = props.order?.status === "OPEN" || props.order?.status === "IN_PROGRESS";

  return (
    <Dialog actions={<>
      {showStart ? <Button disabled={props.isSubmitting} onClick={props.onStart} type="button" variant="secondary"><Play data-icon="inline-start" />In Bearbeitung</Button> : null}
