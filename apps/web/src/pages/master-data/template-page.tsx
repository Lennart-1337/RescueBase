...
export function MasterDataTemplatePage() {
  ...
  const createTemplate = useMutation({ mutationFn: rescueBaseApi.createTemplate, onSuccess: async () => Promise.all([queryClient.invalidateQueries({ queryKey: catalogKeys.templates() }), queryClient.invalidateQueries({ queryKey: catalogKeys.kits() })]) });
  const reviseTemplate = useMutation({ mutationFn: ({ body, id }: Parameters<typeof rescueBaseApi.reviseTemplate> extends [infer I, infer B] ? { id: I & string; body: B } : never) => rescueBaseApi.reviseTemplate(id, body), onSuccess: async () => Promise.all([queryClient.invalidateQueries({ queryKey: catalogKeys.templates() }), queryClient.invalidateQueries({ queryKey: catalogKeys.kits() })]) });
  const deleteTemplate = useMutation({ mutationFn: rescueBaseApi.deleteTemplate, onSuccess: async () => Promise.all([queryClient.invalidateQueries({ queryKey: catalogKeys.templates() }), queryClient.invalidateQueries({ queryKey: catalogKeys.kits() })]) });
