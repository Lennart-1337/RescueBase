...
export function ArticlePanel(props: {
  ...
  const [expandedReorderId, setExpandedReorderId] = useState<string | null>(
    null,
  );
  const canSubmit = Boolean(
    draft.name.trim() &&
    draft.unit.trim() &&
    intervalsValid(draft) &&
    unitsPerPackageValid(draft.unitsPerPackage),
  );
  const categories = [
    ...new Set(
      props.articles
  ...
  async function submit() {
    ...
    const body = {
      ...
      stkIntervalMonths: intervalValue(
        draft.stkRequired,
        draft.stkIntervalMonths,
      ),
      mtkRequired: draft.mtkRequired,
      mtkIntervalMonths: intervalValue(
        draft.mtkRequired,
        draft.mtkIntervalMonths,
      ),
    ...
    const succeeded = await (
      draft.editingId
        ? props.onSave(draft.editingId, body)
        : props.onCreate(body)
    )
      .then(() => true)
      .catch(() => false);
  ...
  return (
    <>
      <PageToolbar label="Artikel filtern">
        <ListFilterBar
          countLabel={`${props.articles.length}/${props.totalCount} sichtbar`}
          fieldsClassName="article-filter-grid"
          onReset={props.onResetFilters}
        >
          <Field label="Suche">
            <input
              onChange={(event) =>
                props.onFilterChange({ q: event.target.value })
              }
              placeholder="Name, Hersteller oder Barcode"
          ...
          <div aria-label="Artikelmerkmale" className="article-filter-checks">
            ...
            <CheckboxField
              ...
              label="Kritisch"
              onChange={(event) =>
                props.onFilterChange({ criticalDefault: event.target.checked })
              }
            />
          </div>
        </ListFilterBar>
      ...
      <Panel>
        <PanelHeader
          className="article-panel-header"
          description={`${props.articles.length}/${props.totalCount} sichtbar`}
          title="Artikel"
          actions={
            <Button onClick={openForCreate} type="button">
        ...
        {props.articles.length === 0 ? (
          <div className="compact-list-empty">Noch keine Artikel angelegt.</div>
        ) : null}
        ...
        <Dialog
          actions={
            <>
              ...
              >
                ...
                {draft.editingId ? "Artikel speichern" : "Artikel anlegen"}
              </Button>
            </>
          }
          ...
          open={draft.isOpen}
          size="wide"
          title={draft.editingId ? "Artikel bearbeiten" : "Artikel anlegen"}
        >
          <div className="article-editor-layout">
            <div className="article-editor-fields">
              <Field className="article-editor-field-name" label="Name">
                <input
                  autoFocus
                  onChange={(event) =>
                    setDraft((current) => ({
              ...
              <Field
                label={
                  draft.unitsPerPackage
                    ? "Standardpreis je VE brutto"
                    : "Standardpreis je Stück brutto"
                }
              >
                <input
                  inputMode="decimal"
                  onChange={(event) =>
                    setDraft((current) => ({
                      ...current,
                      defaultGrossPrice: event.target.value,
                    }))
                  }
                  placeholder="0,00"
                  value={draft.defaultGrossPrice}
                />
              </Field>
              <Field
                className="article-editor-field-storage"
                label="Lagerhinweise"
              >
                <input
                  onChange={(event) =>
                    setDraft((current) => ({
                      ...current,
                      storageNotes: event.target.value,
                    }))
                  }
                  value={draft.storageNotes}
                />
              </Field>
              <Field className="article-editor-field-notes" label="Hinweise">
                <textarea
                  onChange={(event) =>
                    setDraft((current) => ({
                      ...current,
            ...
            <section
              aria-label="Artikelmerkmale"
              className="article-editor-flags"
            >
              <h3>Merkmale</h3>
              <CheckboxField
                checked={draft.sterile}
                className="article-editor-toggle"
                label="Steril"
                onChange={(event) =>
                  setDraft((current) => ({
                    ...current,
                  ...
                  >
                    <input
                      ...
                      onChange={(event) =>
                        setDraft((current) => ({
                          ...current,
                          stkIntervalMonths: event.target.value,
                        }))
                      }
                      type="number"
                      value={draft.stkIntervalMonths}
                    />
                  </Field>
                ) : null}
              </div>
                  ...
                  >
                    <input
                      ...
                      onChange={(event) =>
                        setDraft((current) => ({
                          ...current,
                          mtkIntervalMonths: event.target.value,
                        }))
                      }
                      type="number"
                      value={draft.mtkIntervalMonths}
                    />
                  </Field>
                ) : null}
              </div>
...
function intervalsValid(draft: ArticleDraft) {
  return (
    (!draft.stkRequired ||
      (Number.isInteger(Number(draft.stkIntervalMonths)) &&
        Number(draft.stkIntervalMonths) > 0)) &&
    (!draft.mtkRequired ||
      (Number.isInteger(Number(draft.mtkIntervalMonths)) &&
        Number(draft.mtkIntervalMonths) > 0))
  );
}

function unitsPerPackageValid(value: string) {
  return (
    value.trim() === "" ||
