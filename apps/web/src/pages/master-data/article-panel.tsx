...
export function ArticlePanel(props: {
  ...
  const canSubmit = Boolean(draft.name.trim() && draft.unit.trim());

  function openForCreate() { setDraft({ ...emptyDraft(), isOpen: true }); }
  function openForEdit(article: Article) { setDraft({ ...article, isOpen: true, editingId: article.id, manufacturer: article.manufacturer ?? "", manufacturerPartNumber: article.manufacturerPartNumber ?? "", category: article.category ?? "", barcode: article.barcode ?? "", storageNotes: article.storageNotes ?? "", notes: article.notes ?? "" }); }
  async function submit() {
    if (!canSubmit) return;
    const body = { name: draft.name, unit: draft.unit, manufacturer: optionalText(draft.manufacturer), manufacturerPartNumber: optionalText(draft.manufacturerPartNumber), category: optionalText(draft.category), barcode: optionalText(draft.barcode), sterile: draft.sterile, storageNotes: optionalText(draft.storageNotes), notes: optionalText(draft.notes), criticalDefault: draft.criticalDefault };
  ...
  return (
    <Panel>
      <div className="panel-header"><div><h2>Artikel</h2><p>Materialstamm mit Herstellerdaten, Sterilität und Lagerhinweisen.</p></div><Button onClick={openForCreate} type="button"><Plus data-icon="inline-start" />Artikel hinzufügen</Button></div>
      ...
      <div className="compact-list">
        {props.articles.map((article) => (
          <div className="compact-list-row compact-list-row-actions" key={article.id}>
            <span><strong>{article.name}</strong><small>{[article.unit, article.manufacturer, article.category].filter(Boolean).join(" · ")}</small></span>
            <div className="row-actions">{article.sterile ? <Badge tone="info">steril</Badge> : null}{article.criticalDefault ? <Badge tone="info">kritisch</Badge> : null}<Button onClick={() => openForEdit(article)} type="button" variant="ghost"><Pencil data-icon="inline-start" />Bearbeiten</Button></div>
          </div>
        ))}
      </div>
      <Dialog actions={<><Button disabled={props.isSubmitting} onClick={() => setDraft(emptyDraft())} type="button" variant="ghost"><X data-icon="inline-start" />Abbrechen</Button><Button disabled={!canSubmit || props.isSubmitting} onClick={() => void submit()} type="button">{draft.editingId ? <Save data-icon="inline-start" /> : <Plus data-icon="inline-start" />}{draft.editingId ? "Artikel speichern" : "Artikel anlegen"}</Button></>} description="Pflegen Sie Materialstammdaten für Medizinprodukte und Verbrauchsmaterial." onClose={() => setDraft(emptyDraft())} open={draft.isOpen} title={draft.editingId ? "Artikel bearbeiten" : "Artikel anlegen"}>
        <div className="form-grid form-grid-three">
          ...
          <div className="form-grid"><label className="check-field"><input checked={draft.sterile} onChange={(event) => setDraft((current) => ({ ...current, sterile: event.target.checked }))} type="checkbox" /><span>Steril</span></label><label className="check-field"><input checked={draft.criticalDefault} onChange={(event) => setDraft((current) => ({ ...current, criticalDefault: event.target.checked }))} type="checkbox" /><span>Kritisch als Standard</span></label></div>
        </div>
...
function emptyDraft() {
  return { isOpen: false, editingId: null as string | null, name: "", unit: "Stück", manufacturer: "", manufacturerPartNumber: "", category: "", barcode: "", sterile: false, storageNotes: "", notes: "", criticalDefault: false };
}
