...
export function Devices({ devices, catalog }: { devices: Device[]; catalog: Catalog }) {
  ...
  return <><section className="mpg-section"><header className="mpg-section-header"><div><h2>Geräte</h2><p>{filtered.length} von {devices.length} Geräten</p></div><label className="mpg-search">Suchen<input aria-label="Geräte suchen" type="search" placeholder="Name, Inventar- oder Seriennummer" value={search} onChange={(event) => setSearch(event.target.value)} /></label></header>
    <div className="mpg-actions"><AnchorButton variant="secondary" href="/api/mpg/exports/inventory.pdf">Bestandsverzeichnis PDF</AnchorButton><AnchorButton variant="ghost" href="/api/mpg/exports/inventory.csv">CSV</AnchorButton></div>
    ...
    <MpgDataTable columns={columns} emptyMessage="Keine Geräte für diese Filter gefunden." getRowId={device => device.id} rows={filtered} /><details className="mpg-detail"><summary>Gerät erfassen</summary><MpgForm title="Neues Gerät" fields={deviceFields(catalog)} onSubmit={(body) => action.mutateAsync({ path: "/devices", body })} /></details></section><Models models={catalog.models} />
  </>;
}
