...
export function PersonDetail({ person, users, onDelete }: { person: Person; users: UserSummary[]; onDelete: () => void }) {
  ...
  return <section className="mpg-detail-section"><header><div><h3>{person.name}</h3><p>{person.birthDate ? `Geboren am ${displayDate(person.birthDate)}` : "Geburtsdatum fehlt - bitte ergänzen"} · {person.user ? `verknüpft mit ${person.user.displayName}` : "ohne Benutzerkonto"}</p></div><div className="mpg-row-actions"><AnchorButton variant="ghost" href={`/api/mpg/exports/people/${person.id}.pdf`}>Einweisungen PDF</AnchorButton><Button onClick={() => setEditing(true)} type="button" variant="secondary"><Pencil data-icon="inline-start" />Bearbeiten</Button><Button onClick={onDelete} type="button" variant="danger"><Trash2 data-icon="inline-start" />Löschen</Button></div></header>
