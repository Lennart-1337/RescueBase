...
export function PurchaseOrderEditDialog(props: {
  ...
  return (
    <Dialog
      actions={<><Button onClick={props.onClose} type="button" variant="ghost">Abbrechen</Button><Button disabled={(isDraft && !canSaveDraft) || props.isSaving} onClick={save} type="button">{isDraft ? "Bestellung speichern" : "Hinweise speichern"}</Button></>}
...
function DraftEditor(props: {
  ...
  return <div className="purchase-order-form"><div className="form-grid form-grid-three"><Field label="Lieferant"><input onChange={(event) => props.onSupplierChange(event.target.value)} value={props.supplierName} /></Field><Field label="Zielort"><SearchableSelect onChange={props.onLocationChange} options={props.locations.map((location) => ({ label: location.name, value: location.id }))} value={props.locationId} /></Field><Field label="Hinweise"><input onChange={(event) => props.onNotesChange(event.target.value)} value={props.notes} /></Field></div><div className="purchase-order-lines">{props.lines.map((line, index) => <DraftLineEditor articles={props.articles} index={index} key={index} line={line} onRemove={() => props.onLineRemove(index)} onUpdate={(patch) => props.onLineUpdate(index, patch)} />)}</div><div className="purchase-order-dialog-actions"><Button onClick={props.onAddLine} type="button" variant="secondary">Position hinzufügen</Button></div>{props.error ? <InlineError error={props.error} /> : null}</div>;
}
