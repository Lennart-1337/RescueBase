...
import { changeValue, clickElement, getActiveRouter, mouseDownElement, renderAppAt, resetTestBrowser } from "../test-support/app-test-helpers";
...
describe("KitsPage", () => {
  ...
  it("soft-deletes kits after confirmation", async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (pathname === "/api/auth/setup/status") return jsonResponse({ initialized: true });
      if (pathname === "/api/auth/get-session") return jsonResponse({ user: { id: "user-admin", email: "admin@rescuebase.local", name: "Admin", role: "ADMIN", twoFactorEnabled: false } });
      if (pathname === "/api/catalog/kits" && method === "GET") return jsonResponse([kit]);
      if (pathname === "/api/catalog/locations") return jsonResponse([kit.location]);
      if (pathname === "/api/catalog/templates") return jsonResponse([kit.template]);
    ...
    await waitFor(() =>
      expect(fetchMock.mock.calls).toContainEqual([
        "/api/catalog/kits/kit-rucksack-1",
