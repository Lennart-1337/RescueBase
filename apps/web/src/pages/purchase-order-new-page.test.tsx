...
describe("PurchaseOrderNewPage", () => {
  ...
  it("converts package counts into ordered units for manual purchase orders", async () => {
    stubFetch({
      "/api/auth/setup/status": { initialized: true },
      "/api/auth/session": {
        user: {
  ...
  it("animates the mode switch content without losing the shortage controls", async () => {
    ...
    await renderAppAt("/admin/purchase-orders/new");
    await clickElement(
      await screen.findByRole("tab", { name: "Aus Fehlmengen" }),
    );

    const shortageHeading = await screen.findByRole("heading", {
      name: "Fehlmengen",
    });
    expect(shortageHeading.closest("[data-motion-preset]")).toHaveAttribute(
      "data-motion-preset",
      "panel-enter",
    );
  ...
  it("renders the creation modes above the form using the shared tab style", async () => {
    ...
    expect(
      await screen.findByRole("tab", { name: "Manuell" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("tab", { name: "Aus Fehlmengen" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("tablist", { name: "Erstellmodus" }),
    ).toBeInTheDocument();
  });
  ...
  it("renders the header fields without extra horizontal inset", async () => {
    ...
    expect(
      document.querySelector(".purchase-order-header-grid"),
    ).toBeInTheDocument();
  });
});
