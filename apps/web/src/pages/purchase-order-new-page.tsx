      ...
      <Panel>
        ...
        {mode === "manual" ? (
          <div className="purchase-order-form">
            ...
            <div className="row-action-buttons"><Button onClick={() => setLines((current) => [...current, emptyLine()])} type="button" variant="secondary"><Plus data-icon="inline-start" />Position</Button><Button disabled={!canSubmitManual || createManual.isPending} onClick={submitManual} type="button"><Save data-icon="inline-start" />Entwurf anlegen</Button></div>
          ...
          <div className="purchase-order-form">
            ...
            <Button disabled={!canSubmitShortages || createFromShortages.isPending} onClick={submitShortages} type="button"><Save data-icon="inline-start" />Entwurf anlegen</Button>
...
function OrderHeaderFields(props: { locations: Array<{ id: string; name: string }>; locationId: string; notes: string; onLocationChange: (value: string) => void; onNotesChange: (value: string) => void; onSupplierChange: (value: string) => void; supplierName: string }) {
  return <div className="form-grid form-grid-three"><Field label="Lieferant"><input onChange={(event) => props.onSupplierChange(event.target.value)} value={props.supplierName} /></Field><Field label="Zielort"><SearchableSelect onChange={props.onLocationChange} options={props.locations.map((location) => ({ label: location.name, value: location.id }))} value={props.locationId} /></Field><Field label="Hinweise"><input onChange={(event) => props.onNotesChange(event.target.value)} value={props.notes} /></Field></div>;
}
...
function ManualLine(props: { articles: Article[]; index: number; line: DraftLine; onRemove: () => void; onUpdate: (patch: Partial<DraftLine>) => void }) {
  const article = props.articles.find((entry) => entry.id === props.line.articleId);
  return <div className="purchase-order-line-grid"><Field label="Artikel"><SearchableSelect emptyLabel="Artikel wählen" onChange={(value) => props.onUpdate({ articleId: value, price: centsInput(props.articles.find((entry) => entry.id === value)?.defaultGrossPriceCents), supplierArticleNumber: props.articles.find((entry) => entry.id === value)?.manufacturerPartNumber ?? "" })} options={props.articles.map((entry) => ({ label: entry.name, value: entry.id, keywords: [entry.manufacturer ?? "", entry.barcode ?? ""] }))} value={props.line.articleId} /></Field><Field label="Menge"><input min="1" onChange={(event) => props.onUpdate({ quantity: event.target.value })} type="number" value={props.line.quantity} /></Field><Field label="Preis"><input inputMode="decimal" onChange={(event) => props.onUpdate({ price: event.target.value })} value={props.line.price} /></Field><Field label="Art.-Nr. / Notiz"><input onChange={(event) => props.onUpdate({ note: event.target.value })} placeholder={article?.manufacturerPartNumber ?? ""} value={props.line.note} /></Field><Button aria-label={`Position ${props.index + 1} entfernen`} onClick={props.onRemove} type="button" variant="danger"><Trash2 /></Button></div>;
}
