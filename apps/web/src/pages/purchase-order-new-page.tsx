...
export function PurchaseOrderNewPage() {
  ...
  const search = useSearch({ from: "/admin/purchase-orders/new" }) as {
    mode?: string;
  };
  ...
  const [groupingMode, setGroupingMode] = useState<"single" | "supplier">(
    "single",
  );
  ...
  const createManual = useMutation({
    mutationFn: rescueBaseApi.createPurchaseOrder,
    onSuccess: onCreated,
  });
  const createFromShortages = useMutation({
    mutationFn: rescueBaseApi.createPurchaseOrdersFromShortages,
  ...
  const shortages = targets.data.filter(
    (target) =>
  ...
  return (
    <>
      <div className="purchase-order-page-header">
        <Link className="purchase-order-back-link" to="/admin/purchase-orders">
          ...
          {mode === "manual" ? (
            <div className="purchase-order-form">
              ...
              <section
                ...
                className="purchase-order-new-section"
              >
              ...
              {createManual.error ? (
                <InlineError error={createManual.error} />
              ) : null}
            </div>
          ) : (
            <div className="purchase-order-form">
              ...
              >
                ...
                <Field label="Gruppierung">
                  <SearchableSelect
                    onChange={(value) =>
                      setGroupingMode(value as "single" | "supplier")
                    }
                    options={[
                      { label: "Eine Sammelbestellung", value: "single" },
                      { label: "Nach Lieferant gruppieren", value: "supplier" },
                    ]}
                    value={groupingMode}
                  />
                </Field>
                <div className="purchase-order-lines">
                  {shortages.map((target) => (
                    <ShortageOption
                      key={target.id}
                      onToggle={(checked) =>
                        setShortageArticleIds((current) =>
                          checked
                            ? [...current, target.articleId]
                            : current.filter((id) => id !== target.articleId),
                        )
                      }
                      selected={shortageArticleIds.includes(target.articleId)}
                      target={target}
                    />
                  ))}
                  {shortages.length === 0 ? (
                    <div className="compact-list-empty">
                      Keine Fehlmengen am ausgewählten Zielort.
                    </div>
                  ) : null}
                </div>
              </section>
              ...
              {createFromShortages.error ? (
                <InlineError error={createFromShortages.error} />
              ) : null}
            </div>
          )}
        </AnimatedContentSwap>
...
function OrderHeaderFields(props: {
  ...
  return (
    <section aria-label="Bestellkopf" className="purchase-order-new-section">
      <div className="purchase-order-new-section-header">
        <div>
          <h2>Rahmendaten</h2>
          <p>Lieferant, Zielort und Hinweise für den Entwurf.</p>
        </div>
      </div>
...
function ManualLine(props: {
  ...
  const article = props.articles.find(
    (entry) => entry.id === props.line.articleId,
  );
  const unitsPerPackage = article?.unitsPerPackage ?? null;

  return (
    <div className="purchase-order-new-line">
      <div className="purchase-order-new-line-header">
        <strong>Position {props.index + 1}</strong>
        <Button
          aria-label={`Position ${props.index + 1} entfernen`}
          className="purchase-order-line-remove"
      ...
      <div className="purchase-order-line-editor">
        <Field label="Artikel">
          <SearchableSelect
            emptyLabel="Artikel wählen"
            onChange={(value) => {
              const selectedArticle = props.articles.find(
                (entry) => entry.id === value,
              );
              const defaultUnitsPerPackage =
                selectedArticle?.unitsPerPackage ?? null;
              props.onUpdate({
                articleId: value,
                price: centsInput(selectedArticle?.defaultGrossPriceCents),
                quantity: defaultUnitsPerPackage
                  ? String(defaultUnitsPerPackage)
                  : props.line.quantity,
                packageCount: defaultUnitsPerPackage ? "1" : "",
                supplierArticleNumber:
                  selectedArticle?.manufacturerPartNumber ?? "",
              });
            }}
            options={props.articles.map((entry) => ({
              label: entry.name,
              value: entry.id,
              keywords: [entry.manufacturer ?? "", entry.barcode ?? ""],
            }))}
            value={props.line.articleId}
          />
        </Field>
        {unitsPerPackage ? (
            ...
            <input
              ...
              onChange={(event) =>
                props.onUpdate({
                  ...
                  quantity: deriveQuantityFromPackages(
                    event.target.value,
                    unitsPerPackage,
                  ),
                })
          ...
          <input
            ...
            onChange={(event) =>
              props.onUpdate({
                ...
                packageCount: derivePackageCount(
                  event.target.value,
                  unitsPerPackage,
                ),
              })
        ...
        {unitsPerPackage ? (
          <p className="form-hint purchase-order-package-hint">
        ...
        <Field label="Art.-Nr.">
          <input
            onChange={(event) =>
              props.onUpdate({ supplierArticleNumber: event.target.value })
            }
            value={props.line.supplierArticleNumber}
          />
        </Field>
        <Field label="Notiz">
          <input
            onChange={(event) => props.onUpdate({ note: event.target.value })}
            value={props.line.note}
          />
        </Field>
      </div>
    </div>
...
function deriveQuantityFromPackages(
  packageCountValue: string,
  unitsPerPackage: number,
) {
  const normalized = Number.parseInt(packageCountValue, 10);
  return Number.isInteger(normalized) && normalized > 0
    ? String(normalized * unitsPerPackage)
    : "";
}

function derivePackageCount(
  quantityValue: string,
  unitsPerPackage: number | null,
) {
  if (!unitsPerPackage) return "";
  const normalized = Number.parseInt(quantityValue, 10);
  if (
    !Number.isInteger(normalized) ||
    normalized <= 0 ||
    normalized % unitsPerPackage !== 0
  )
    return "";
  return String(normalized / unitsPerPackage);
}
