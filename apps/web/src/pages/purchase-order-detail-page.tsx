    ...
    <>
      ...
      <Workspace className="purchase-order-workspace">
        ...
        <WorkspaceRail className="purchase-order-rail" label="Bestellkontext">
          <PurchaseOrderDetailRail canApprove={canApprove} includeLineNotes={includeLineNotes} isApproving={approve.isPending} isArchiving={archive.isPending} isOrdering={markOrdered.isPending} isRestoring={restore.isPending} onApprove={() => approve.mutate()} onArchive={() => archive.mutate()} onEdit={() => setEditingOrder(true)} onIncludeNotesChange={setIncludeLineNotes} onMarkOrdered={() => markOrdered.mutate()} onRestore={() => restore.mutate()} order={current} pdfHref={pdfHref} />
