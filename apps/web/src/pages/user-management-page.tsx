...
import type { AuthenticatedUser, MedicalDevicePermissionUser, UserSummary } from "../lib/types";
...
import { UserAccessPermission } from "./users/user-access-permission";
...
export function UserManagementPage({ user }: { user: AuthenticatedUser }) {
  ...
  const permissions = useQuery({ queryKey: ["mpg-permissions", "users"], queryFn: rescueBaseApi.medicalDevicePermissionUsers, enabled: user.role === "ADMIN" });
  ...
  const updatePermission = useMutation({ mutationFn: ({ enabled, id }: { enabled: boolean; id: string }) => rescueBaseApi.updateMedicalDevicePermission(id, enabled), onSuccess: () => queryClient.invalidateQueries({ queryKey: ["mpg-permissions", "users"] }) });
  ...
  return <>
    ...
    <div className="user-management-layout">
      ...
      <AccountDetails currentUserId={user.id} detailTab={detailTab} isSubmitting={toggle.isPending || update.isPending || security.isPending || deleteUser.isPending || resendInvitation.isPending || revokeInvitation.isPending} onDelete={() => selected && deleteUser.mutate(selected.id)} onEdit={() => setProfileOpen(true)} onPermissionChange={(enabled) => selected && updatePermission.mutate({ id: selected.id, enabled })} onResendInvitation={() => selected && resendInvitation.mutate(selected.id)} onRevokeInvitation={() => selected && revokeInvitation.mutate(selected.id)} onSecurity={() => setSecurityOpen(true)} onTabChange={setDetailTab} onToggle={() => selected && toggle.mutate({ id: selected.id, active: !selected.active })} permission={permissions.data?.find((entry) => entry.id === selected?.id)} permissionError={permissions.isError} permissionPending={updatePermission.isPending} selected={selected} />
    </div>
...
function AccountDetails(props: { currentUserId: string; detailTab: string; isSubmitting: boolean; onDelete: () => void; onEdit: () => void; onPermissionChange: (enabled: boolean) => void; onResendInvitation: () => void; onRevokeInvitation: () => void; onSecurity: () => void; onTabChange: (value: string) => void; onToggle: () => void; permission: MedicalDevicePermissionUser | undefined; permissionError: boolean; permissionPending: boolean; selected: UserSummary | null }) {
