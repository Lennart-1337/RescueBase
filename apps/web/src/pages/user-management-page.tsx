...
function AccountDetails(props: { currentUserId: string; detailTab: string; isSubmitting: boolean; onDelete: () => void; onEdit: () => void; onResendInvitation: () => void; onRevokeInvitation: () => void; onSecurity: () => void; onTabChange: (value: string) => void; onToggle: () => void; selected: UserSummary | null }) {
  ...
  return <section aria-labelledby="details-heading" className="user-management-details"><header><div><h2 id="details-heading">Kontodetails</h2><p>{selected ? `${selected.displayName} · ${selected.email}` : "Benutzerkonto auswählen"}</p></div>{selected ? <UserRoundCheck /> : null}</header>{selected ? <><Tabs items={[{ label: "Profil", value: "profile" }, { label: "Zugang", value: "access" }, { label: "Sicherheit", value: "security" }]} label="Kontodetails" onChange={props.onTabChange} value={props.detailTab} /><DetailContent tab={props.detailTab} user={selected} /><div className="user-management-actions">{selected.id !== props.currentUserId ? <><Button disabled={props.isSubmitting} onClick={props.onEdit} type="button" variant="secondary">Profil bearbeiten</Button><Button disabled={props.isSubmitting} onClick={props.onSecurity} type="button" variant="secondary"><KeyRound data-icon="inline-start" />Sicherheit verwalten</Button><Button disabled={props.isSubmitting} onClick={props.onToggle} type="button" variant={selected.active ? "danger" : "secondary"}>{selected.active ? "Konto deaktivieren" : "Konto aktivieren"}</Button>{selected.invitationStatus === "OPEN" || selected.invitationStatus === "EXPIRED" || selected.invitationStatus === "REVOKED" ? <Button disabled={props.isSubmitting} onClick={props.onResendInvitation} type="button" variant="secondary"><Send data-icon="inline-start" />Einladung senden</Button> : null}{selected.invitationStatus === "OPEN" ? <Button disabled={props.isSubmitting} onClick={confirmRevoke} type="button" variant="secondary"><ShieldX data-icon="inline-start" />Einladung widerrufen</Button> : null}<Button disabled={props.isSubmitting} onClick={confirmDelete} type="button" variant="danger"><Trash2 data-icon="inline-start" />Konto löschen</Button></> : null}</div></> : null}</section>;
}
