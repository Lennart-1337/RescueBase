...
export async function getAlertOverview() {
  return requestJson<{ generatedAt: string; warnings: AlertWarning[]; summary: { expiry: number; stkDue: number; mtkDue: number; shortage: number; kitCheckDue: number } }>("/alerts/warnings");
}
