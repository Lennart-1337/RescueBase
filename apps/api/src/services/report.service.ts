...
export class ReportService {
  ...
  async kitTemplatePdf(templateId: string): Promise<Buffer> {
    const template: KitTemplatePdfRecord | null =
      await this.prisma.kitTemplate.findFirst({
    ...
    return this.renderPdf((doc) => {
      this.drawDocumentHeader(
        ...
        "Rucksackvorlage",
        template.name,
        `Version ${template.version} · ${template.positions.length} Positionen`,
      );
      this.drawSummaryLine(doc, [
        `${groups.length} Module`,
        `${template.positions.length} Positionen`,
        `${template.positions.filter((position) => position.critical).length} kritisch`,
      ]);
      ...
      for (const group of groups) {
        if (
          ...
          this.drawContinuationHeader(
            doc,
            template.name,
            `Version ${template.version} · Inhaltsliste`,
          );
          y = doc.y + 12;
        }
        ...
        for (const position of group.positions) {
          ...
          if (
            ...
            this.drawContinuationHeader(
              doc,
              template.name,
              `Version ${template.version} · ${group.name}`,
            );
          ...
          doc
            ...
            .font("Helvetica")
            .fontSize(9)
            .text(position.article.name, 48, y, { width: 290 });
          doc.text(String(position.requiredQuantity), 344, y, {
            width: 48,
            align: "right",
          });
          ...
          if (position.critical)
            doc
              ...
              .font("Helvetica-Bold")
              .fontSize(8)
              .text("Kritisch", 486, y + 1, { width: 50 });
          doc
            .moveTo(48, y + rowHeight + 5)
            .lineTo(doc.page.width - 48, y + rowHeight + 5)
  ...
  async procurementPdf(filters: ProcurementPdfFilters): Promise<Buffer> {
    const orders: ProcurementOrderReportRecord[] =
      await this.prisma.inventoryProcurementOrder.findMany({
        where: {
          status: {
            in: [
    ...
    const filtered = orders.filter((order) =>
      matchesProcurementFilters(filters, order),
    );
    const totalRemaining = filtered.reduce(
      (sum, order) =>
        sum + Math.max(order.requestedQuantity - order.receivedQuantity, 0),
      0,
    );
    const filterLabel = summarizeProcurementFilters(filters);

    return this.renderPdf((doc) => {
      this.drawProcurementHeader(
        doc,
        filterLabel,
        totalRemaining,
        filtered.length,
  ...
  async qrLabelPdf(kitId: string, format: QrPdfFormat): Promise<Buffer> {
    ...
    return this.renderPdf((doc) =>
      this.drawQrSheet(doc, kit, publicUrl, qrImage),
    );
  }
  ...
  async replenishmentPdf(orderId: string): Promise<Buffer> {
    ...
    return this.renderPdf((doc) => {
      ...
      const totalRequested = order.items.reduce(
        (sum: number, item: OrderRecord["items"][number]) =>
          sum + item.requestedQuantity,
        0,
      );
      const totalFulfilled = order.items.reduce(
        (sum: number, item: OrderRecord["items"][number]) =>
          sum + item.fulfilledQuantity,
        0,
      );
      this.drawSummaryLine(doc, [
      ...
      order.items.forEach(
        (item: OrderRecord["items"][number], index: number) => {
          ...
          if (
            ...
            this.drawContinuationHeader(
              doc,
              "Nachfüllauftrag",
              `Auftrag ${order.id} · Fortsetzung`,
            );
            y = this.drawTableHeader(doc, doc.y + 14, [
              "Artikel",
              "Grund",
              "Soll",
              "Erfüllt",
  ...
  async purchaseOrderPdf(
    orderId: string,
    options: { includeLineNotes?: boolean },
  ): Promise<Buffer> {
    const order: PurchaseOrderPdfRecord | null =
    ...
    return this.renderPdf((doc) => {
      ...
      this.drawMetaGrid(
        ...
        [
          ...
          [
            "Freigabe",
            order.approvedAt
              ? `${order.approvedByName ?? "Unbekannt"} · ${formatDateTime(order.approvedAt)}`
      ...
      const totalGrossCents = order.lines.reduce(
        (sum, line) => sum + calculatePurchaseOrderLineTotal(line),
        0,
      );
      ...
      if (order.notes) {
        ...
        const noteHeight = doc
          .font("Helvetica")
          .fontSize(9)
          .heightOfString(order.notes, { width: doc.page.width - 96 });
        doc
          .fillColor(palette.muted)
          .font("Helvetica-Bold")
          .fontSize(8)
          .text("BESTELLHINWEISE", 48, noteTop);
        doc
          .fillColor(palette.ink)
          .font("Helvetica")
          .fontSize(9)
          .text(order.notes, 48, noteTop + 14, { width: doc.page.width - 96 });
        doc
          .moveTo(48, noteTop + noteHeight + 24)
          .lineTo(doc.page.width - 48, noteTop + noteHeight + 24)
          .strokeColor(palette.line)
          .stroke();
      ...
      order.lines.forEach((line, index) => {
        const rowHeight = this.measurePurchaseOrderLine(
          doc,
          line,
          Boolean(options.includeLineNotes),
        );
        if (
          needsPageBreak({
            currentY: y,
          ...
          this.drawContinuationHeader(
            doc,
            "Bestellung",
            `${order.orderNumber} · Fortsetzung`,
          );
          y = this.drawPurchaseOrderTableHeader(doc, doc.y + 12);
        }
        y = this.drawPurchaseOrderLine(
          doc,
          y,
          line,
          index,
  ...
  private drawQrSheet(
    ...
    this.drawMetaGrid(
      ...
      {
        x: layout.leftColumn.x,
        y: layout.leftColumn.y + 56,
        width: layout.leftColumn.width,
      },
    );
    ...
    const urlHeight = doc
      .font("Helvetica")
      .fontSize(9)
      .heightOfString(publicUrl, { width: doc.page.width - 96 });
    doc
      .fillColor(palette.muted)
      .font("Helvetica-Bold")
      .fontSize(8)
      .text("DIREKTLINK", 48, linkTop);
    doc
      .fillColor(palette.ink)
      .font("Helvetica")
      .fontSize(9)
      .text(publicUrl, 48, linkTop + 14, {
        width: doc.page.width - 96,
      });
    doc
      .fillColor(palette.muted)
    ...
    doc
      .moveTo(48, linkTop + 58 + urlHeight)
      .lineTo(doc.page.width - 48, linkTop + 58 + urlHeight)
      .strokeColor(palette.line)
      .stroke();
  }
  ...
  private drawQrLabel(
    doc: PDFKit.PDFDocument,
    kit: { name: string; code: string; location: { name: string } },
    qrImage: Buffer,
  ) {
    ...
    const location = fitSingleLine(
      doc.font("Helvetica").fontSize(5.8),
      kit.location.name,
      layout.metaBox.width,
    );
    ...
    doc
      ...
      .fillAndStroke(palette.paper, palette.lineStrong);

    doc
      .fillColor(palette.muted)
      .font("Helvetica-Bold")
      .fontSize(5.2)
      .text("CHECK", layout.textBox.x, layout.textBox.y, {
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(8.8)
      .text(code, layout.codeBox.x, layout.codeBox.y, {
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(5.6)
      .text(name, layout.textBox.x, layout.textBox.y + 24, {
        width: layout.textBox.width,
      });
    doc
      .fillColor(palette.muted)
      .font("Helvetica")
      .fontSize(5.8)
      .text(location, layout.metaBox.x, layout.metaBox.y + 1, {
    ...
    doc
      .roundedRect(
        layout.qrBox.x - 3,
        layout.qrBox.y - 3,
        layout.qrBox.width + 6,
        layout.qrBox.height + 6,
        4,
      )
      .fillAndStroke(palette.paper, palette.lineStrong);
    doc.image(qrImage, layout.qrBox.x, layout.qrBox.y, {
      width: layout.qrBox.width,
      height: layout.qrBox.height,
    });
    doc
      .fillColor(palette.muted)
      .font("Helvetica-Bold")
      .fontSize(5.2)
      .text("Scan für Check", layout.qrCaptionBox.x, layout.qrCaptionBox.y, {
  ...
  private drawMetaGrid(
    ...
    drawInfoGrid(doc, rows, {
      ...options,
      columns: options.width < 340 ? 1 : 2,
    });
  }
  ...
  private drawQrCard(
    ...
    doc
      .rect(box.x - 12, box.y - 12, box.width + 24, box.height + 42)
      .strokeColor(palette.line)
      .stroke();
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(8)
      .text("Öffentlicher Check", box.x, box.y + box.height + 10, {
        width: box.width,
        align: "center",
      });
  }
  ...
  private measureReplenishmentRow(
    ...
    const articleHeight = doc
      .font("Helvetica-Bold")
      .fontSize(10)
      .heightOfString(item.articleName, { width: 190 });
    const reasonHeight = doc
      .font("Helvetica")
      .fontSize(9)
      .heightOfString(formatReason(item.reason), { width: 92 });
  ...
  private drawReplenishmentRow(
    ...
    if (index % 2 === 0) {
      doc
        .rect(48, y - 4, doc.page.width - 96, rowHeight + 6)
        .fill(palette.panelSoft);
    }
    doc
      .fillColor(item.critical ? palette.critical : palette.ink)
      .font("Helvetica-Bold")
      .fontSize(10)
      .text(item.articleName, 56, y, { width: 190 });
    doc
      .fillColor(palette.muted)
      .font("Helvetica")
      .fontSize(8)
      .text(
        item.critical ? `Kritisch · ${item.unit}` : item.unit,
        56,
        y + rowHeight - 14,
        {
          width: 190,
    ...
    doc
      ...
      .font("Helvetica")
      .fontSize(9)
      .text(formatReason(item.reason), 255, y + 2, { width: 92 });
    doc.text(String(item.requestedQuantity), 358, y + 2, {
      width: 40,
      align: "right",
    });
    doc.text(String(item.fulfilledQuantity), 418, y + 2, {
      width: 44,
      align: "right",
    });
    doc
      ...
      .text(
        String(item.requestedQuantity - item.fulfilledQuantity),
        482,
        y + 2,
        { width: 48, align: "right" },
      );
    doc
      .moveTo(48, y + rowHeight + 8)
      .lineTo(doc.page.width - 48, y + rowHeight + 8)
      .strokeColor(palette.line)
      .stroke();
  ...
  private drawOperationalNotes(doc: PDFKit.PDFDocument, y: number) {
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(8)
      .text("OPERATIVE HINWEISE", 48, y);
    doc
      .fillColor(palette.ink)
      .font("Helvetica")
      .fontSize(9)
      .text(
        "Chargen bitte im System buchen. Teilfüllungen und Restmengen müssen nachvollziehbar bleiben.",
        48,
        y + 14,
        {
          width: doc.page.width - 96,
    ...
    doc
      .moveTo(320, y + 100)
      .lineTo(doc.page.width - 48, y + 100)
      .strokeColor(palette.line)
      .stroke();
    doc
      .fillColor(palette.muted)
      .fontSize(8)
      .text("Bearbeitet von", 48, y + 104)
      .text("Datum / Unterschrift", 320, y + 104);
  }

  private drawProcurementHeader(
    doc: PDFKit.PDFDocument,
    filterLabel: string,
    ...
    this.drawDocumentHeader(
      ...
      "Beschaffung",
      "Einkaufsliste",
      "Offene Beschaffungsaufträge für den Einkauf.",
    );
    this.drawMetaGrid(
      doc,
      [
  ...
  private drawPurchaseOrderTableHeader(doc: PDFKit.PDFDocument, y: number) {
    return drawTableHeader(doc, y, [
      ...
      { label: "PREIS / VE", x: 366, width: 64, align: "right" },
  ...
  private drawPurchaseOrderLetterBlock(
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(8)
      .text("ABSENDER", leftX, top, { width: columnWidth });
    doc
      .fillColor(palette.muted)
      .font("Helvetica-Bold")
      .fontSize(8)
      .text("LIEFERANT", rightX, top, { width: columnWidth });
    ...
    doc
      ...
      .font("Helvetica")
      .fontSize(10)
      .text(leftText, leftX, contentTop, { width: columnWidth });
  ...
  private measurePurchaseOrderLine(
    ...
    const articleHeight = doc
      .font("Helvetica-Bold")
      .fontSize(10)
      .heightOfString(line.articleNameSnapshot, { width: 176 });
    const noteHeight =
      includeLineNotes && line.note
        ? doc
            .font("Helvetica")
            .fontSize(8)
            .heightOfString(line.note, { width: 454 }) + 10
        : 0;
  ...
  private drawPurchaseOrderLine(
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(10)
      .text(line.articleNameSnapshot, 48, y, { width: 176 });
    doc
      .font("Helvetica")
      .fontSize(8)
      .fillColor(palette.muted)
      .text(line.unitSnapshot, 48, y + 18, { width: 176 });
    doc.fillColor(palette.ink).font("Helvetica").fontSize(9);
    doc.text(line.supplierArticleNumberSnapshot ?? "-", 232, y + 2, {
      width: 70,
    });
    doc.text(String(line.orderedQuantity), 310, y + 2, {
      width: 48,
      align: "right",
    });
    doc.text(formatCents(line.grossUnitPriceCents), 366, y + 2, {
      width: 64,
      align: "right",
    });
    doc
      .font("Helvetica-Bold")
      .text(formatCents(calculatePurchaseOrderLineTotal(line)), 438, y + 2, {
        width: 64,
        align: "right",
      });
    if (line.articleUrlSnapshot) {
      ...
      doc
        ...
        .font("Helvetica")
        .fontSize(9)
        .text("-", 510, y + 2, { width: 34, align: "right" });
    }
    if (includeLineNotes && line.note) {
      doc
        ...
        .font("Helvetica")
        .fontSize(8)
        .text(line.note, 48, y + 34, { width: 454 });
    }
    doc
      .moveTo(48, y + rowHeight + 8)
      .lineTo(doc.page.width - 48, y + rowHeight + 8)
  ...
  private measureProcurementOrderRow(
    doc: PDFKit.PDFDocument,
    order: ProcurementOrderReportRecord,
  ) {
    const articleHeight = doc
      .font("Helvetica-Bold")
      .fontSize(11)
      .heightOfString(order.article.name, { width: 170 });
    const detailHeight = doc
      .font("Helvetica")
      .fontSize(8)
      .heightOfString(procurementDetails(order).join("\n"), { width: 232 });
  ...
  private drawProcurementOrderRow(
    doc: PDFKit.PDFDocument,
    y: number,
    order: ProcurementOrderReportRecord,
    index: number,
    ...
    const remainingQuantity = Math.max(
      order.requestedQuantity - order.receivedQuantity,
      0,
    );
    const articleUrl = order.article.articleUrl ?? order.articleUrlSnapshot;
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(11)
      .text(order.article.name, 48, y, { width: 170 });
    doc
      .font("Helvetica")
      .fontSize(8)
      .fillColor(palette.ink)
      .text(detailText, 230, y, { width: 232 });
    doc
      .fillColor(palette.ink)
      .font("Helvetica-Bold")
      .fontSize(16)
      .text(String(remainingQuantity), 478, y + 2, {
        width: 56,
        align: "right",
      });
    doc
      ...
      .fontSize(8)
      .fillColor(palette.muted)
      .text(order.article.unit, 478, y + 22, { width: 56, align: "right" });
    doc
      .font("Helvetica-Bold")
      .fontSize(7)
      .fillColor(palette.muted)
      .text(formatProcurementStatus(order.status), 478, y + 34, {
        width: 56,
        align: "right",
      });
  ...
  private templatePositionRowHeight(
    doc: PDFKit.PDFDocument,
    position: KitTemplatePdfRecord["positions"][number],
  ) {
    return Math.max(
      14,
      doc
        .font("Helvetica")
        .fontSize(9)
        .heightOfString(position.article.name, { width: 290 }),
    );
  }

  private drawTemplateModuleHeader(
    doc: PDFKit.PDFDocument,
    y: number,
    moduleName: string,
  ) {
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(10)
      .text(moduleName, 48, y, { width: 300 });
...
function groupTemplatePositions(positions: KitTemplatePdfRecord["positions"]) {
  ...
  return [...groups.entries()]
    ...
    .map(([name, modulePositions]) => ({
      name,
      positions: [...modulePositions].sort((left, right) =>
        left.article.name.localeCompare(right.article.name, "de"),
      ),
    }));
}
...
function calculatePurchaseOrderLineTotal(line: {
  grossUnitPriceCents: number;
  orderedQuantity: number;
  unitsPerPackageSnapshot: number | null;
}) {
...
function procurementDetails(order: ProcurementOrderReportRecord) {
  ...
  if (order.article.manufacturer)
    details.push(`Hersteller: ${order.article.manufacturer}`);
  if (order.article.manufacturerPartNumber)
    details.push(`Art.-Nr.: ${order.article.manufacturerPartNumber}`);
  return details;
}

function matchesProcurementFilters(
  filters: ProcurementPdfFilters,
  order: ProcurementOrderReportRecord,
) {
  if (filters.articleId && filters.articleId !== order.articleId) return false;
  if (filters.locationId && filters.locationId !== order.locationId)
    return false;
